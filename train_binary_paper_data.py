#!/usr/bin/env python3
"""
train_binary_paper_data.py
==========================
Trains our BinaryURLPhishNet on the EXACT SAME dataset as Sabir et al.:
  - Leg_Training.csv  (legitimate URLs from the replication package)
  - Phish_Training.csv (phishing URLs from the replication package)

80/20 stratified split, same as paper's methodology.
This gives a DIRECT, apples-to-apples comparison against Table 7
of arXiv:2005.08454.

After training, tests on their adversarial dataset too — so we can show
both the Original Performance and Adversarial Performance columns.

Outputs:
  binary_paper_data/models/model_best.pth
  binary_paper_data/models/scaler.pkl
  binary_paper_data/logs/run.log
  binary_paper_data/logs/training_log.csv
  binary_paper_data/logs/comparison_vs_table7.txt
  binary_paper_data/logs/adversarial_binary_<type>.csv
"""
...
OUT_MODELS = os.path.join(PROJECT_DIR, "binary_paper_data", "models")
OUT_LOGS   = os.path.join(PROJECT_DIR, "binary_paper_data", "logs")
...
REPORT_TXT  = os.path.join(OUT_LOGS, "comparison_vs_table7.txt")
...
if __name__ == "__main__":
    ...
    log.info("DATA: Sabir et al. replication package — same dataset as Table 7")

    # 1. Load paper's training data
    log.info("[1/6] Loading Leg_Training.csv + Phish_Training.csv...")
    ...
    log.info("[2/6] Extracting 67-dim URL features...")
    extractor = URLFeatureExtractorV2()
    log.info(f"  Feature dim: {extractor.n_features}")
    X_feat, X_char = extract_batch(all_urls, extractor, desc="Extract features")
    log.info(f"  X_feat: {X_feat.shape}  X_char: {X_char.shape}")

    # 3. 80/20 stratified split — mirrors paper's methodology
    log.info("[3/6] 80/20 stratified split (same methodology as paper)...")
    ...
    y_tr,  y_te  = all_labels[tr_idx], all_labels[te_idx]
    ...
    # 4. Train
    log.info(f"[4/6] Training BinaryURLPhishNet on paper's dataset ({EPOCHS} epochs)...")
    ...
    log.info(f"  Best model: epoch {best_epoch}  F1={best_f1:.4f}  MCC={best_f1:.4f}")

    # 5. Adversarial evaluation
    log.info("[5/6] Adversarial evaluation on paper's attack datasets...")
    ...
    # 6. Table 7 comparison report
    log.info("[6/6] Building Table 7 comparison report...")
    sep = "=" * 80
    ...
    lines = [sep,
             "OUR BinaryURLPhishNet vs. Sabir et al. Table 7",
             f"Training data: paper's Leg_Training ({n_leg:,}) + Phish_Training ({n_phish:,})",
